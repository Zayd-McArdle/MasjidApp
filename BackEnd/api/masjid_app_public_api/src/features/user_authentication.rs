...
pub async fn new_user_repository(db_type: DbType) -> Arc<dyn UserRepository> {
    match db_type {
        DbType::InMemory => Arc::new(InMemoryRepository::new(RepositoryType::Announcement).await),
        DbType::MySql => Arc::new(MySqlRepository::new(RepositoryType::Authentication).await),
    }
}
#[async_trait]
...
pub(crate) async fn reset_user_password(
    ...
    let mut password_reset_result = state
        ...
        .await;
    if password_reset_result.is_err() {
        password_reset_result = state
            .repository_map
            .get(&DbType::MySql)
            .unwrap()
            .reset_user_password(&request.username, &request.replacement_password)
...
mod tests {
    ...
    struct TestCase<TRequest, TOk, TErr> {
        request: TRequest,
        expected_in_memory_db_response: Option<Result<TOk, TErr>>,
    ...
    async fn test_login() {
        ...
        let test_cases = vec![
            ...
            TestCase {
                ...
                expected_in_memory_db_response: Some(Err(LoginError::UnableToLogin)),
            ...
            //Given the request body is valid but unable to login via the in-memory database but main database successfully validates credentials, I should get an OK response
            TestCase {
                request: valid_request.clone(),
                expected_in_memory_db_response: Some(Err(LoginError::UnableToLogin)),
            ...
            TestCase {
                request: valid_request.clone(),
                expected_in_memory_db_response: Some(Ok("Admin".to_owned())),
                expected_db_response: None,
                expected_status_code: StatusCode::OK,
            },
            TestCase {
                request: valid_request.clone(),
                expected_in_memory_db_response: Some(Ok("Imam".to_owned())),
                expected_db_response: None,
                expected_status_code: StatusCode::OK,
            },
        ];
        for test_case in test_cases {
            let mut mock_in_memory_repository = MockUserRepository::new();
            ...
            if let Some(expected_in_memory_db_response) = test_case.expected_in_memory_db_response {
                mock_in_memory_repository
                    .expect_login()
                    .returning(move |username, password| expected_in_memory_db_response.clone());
            }
            ...
            let arc_in_memory_repository: Arc<dyn UserRepository> =
    ...
    async fn test_register_user() {
        ...
        let test_cases = vec![
            ...
            TestCase {
                request: valid_request.clone(),
                expected_in_memory_db_response: Some(Err(RegistrationError::FailedToRegister)),
                expected_db_response: Some(Err(RegistrationError::FailedToRegister)),
                expected_status_code: StatusCode::INTERNAL_SERVER_ERROR,
            },
            TestCase {
                request: valid_request.clone(),
                expected_in_memory_db_response: Some(Err(RegistrationError::UserAlreadyRegistered)),
        ...
        for test_case in test_cases {
            let mut mock_in_memory_user_repository = MockUserRepository::new();
            let mut mock_user_repository = MockUserRepository::new();
            if let Some(expected_in_memory_db_response) = test_case.expected_in_memory_db_response {
                mock_in_memory_user_repository
                    .expect_register_user()
                    .returning(move |dto| expected_in_memory_db_response.clone());
            }
            if let Some(expected_db_response) = test_case.expected_db_response {
            ...
            let arc_in_memory_repository: Arc<dyn UserRepository> =
                Arc::new(mock_in_memory_user_repository);
