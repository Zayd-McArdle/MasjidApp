use crate::features::donation::errors::{RefundFailedDonationError, SendDonationServiceError};
...
use masjid_app_api_library::shared::services::email::r#trait::EmailService;
...
use masjid_app_api_library::shared::services::payment::service::PaymentService;
...
pub trait DonationPublicService: Send + Sync {
    ...
    async fn refund_failed_donation(
        &self,
        donation_details: &DonationDetails,
        card_details: &CardDetails,
        billing_address: &BillingAddress,
        is_email_requested: bool,
    ) -> Result<(), RefundFailedDonationError>;
}
...
impl DonationPublicService for DonationServiceImpl<dyn DonationHistoryPublicRepository> {
    async fn send_donation(
        ...
        let transaction_status = self
            .payment_service
            .pay(donation_details.amount, &card_details, &billing_address)
            .await
            .map_err(|err| SendDonationServiceError::PaymentServiceError(err))?;

        let donation_transaction_history = DonationHistory::new(
            donation_details.clone(),
            ...
            .await
        {
            self.payment_service
                .refund(donation_details.amount, &card_details, &billing_address)
                .await
                .map_err(|err| SendDonationServiceError::PaymentServiceError(err))?;
        }
        ...
        if is_email_requested {
            self.email_service
                .send_email(donation_transaction_history.into())
                .await
                .map_err(|err| SendDonationServiceError::EmailServiceFailure(err))?;
        }
...
mod test {
    ...
    use masjid_app_api_library::shared::services::email::r#trait::MockEmailService;
    use masjid_app_api_library::shared::services::payment::card_details::ExpiryDate;
    use masjid_app_api_library::shared::services::payment::errors::PaymentServiceError;
    use masjid_app_api_library::shared::services::payment::service::MockPaymentService;
    use masjid_app_api_library::shared::services::payment::transaction_status::transaction_declined_reason::TransactionDeclinedReason;
    ...
    async fn test_donation_public_service_send_donation() {
        ...
        for test_case in test_cases {
            ...
            mock_payment_service
                .expect_pay()
                .returning(move |_, _, _| test_case.expected_payment_service_result);
            ...
            let donation_service: DonationServiceImpl<dyn DonationHistoryPublicRepository> =
                DonationServiceImpl {
                    ...
                    email_service: Arc::new(mock_email_service),
