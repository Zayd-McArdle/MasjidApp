...
pub async fn send_donation(
    ...
    let send_donation_result = app_state
        .service
        .send_donation(
            &request.donation_details,
            &request.card_details,
            &request.billing_address,
            request.is_email_requested,
        )
        .await;
    match send_donation_result {
        Ok(transaction_status) => Ok(Json(SendDonationResponse::from(transaction_status))),
        Err(err) => match err {
            SendDonationServiceError::DonationHistoryPublicRepositoryError(_)
            | SendDonationServiceError::DonationHistoryPublicInMemoryRepositoryError(_) => {
                app_state
                    .service
                    .refund_failed_donation(
                        &request.donation_details,
                        &request.card_details,
                        &request.billing_address,
                        request.is_email_requested,
                    ...
                    .await
                    .map_err(|e| match e {
                        RefundFailedDonationError::PaymentServiceError(_) => (
            ...
            SendDonationServiceError::PaymentServiceError(_) => Err((
                StatusCode::SERVICE_UNAVAILABLE,
                SERVICE_UNAVAILABLE_RESPONSE_MESSAGE.to_owned(),
            )),
            SendDonationServiceError::EmailServiceFailure(_) => {
                todo!()
            }
        },
    }
    /*.map_err(|err| match err {
        ...
        SendDonationServiceError::DonationHistoryPublicRepositoryError(_) | SendDonationServiceError::DonationHistoryPublicInMemoryRepositoryError(_) => {
...
mod tests {
    ...
    use masjid_app_api_library::shared::services::payment::card_details::{CardDetails, ExpiryDate};
        ...
        for test_case in test_cases {
            ...
            if let Some(expected_service_response) = test_case.expected_service_response {
                mock_donation_service
                    ...
                    .returning(move |_, _, _, _| expected_service_response);
            }
