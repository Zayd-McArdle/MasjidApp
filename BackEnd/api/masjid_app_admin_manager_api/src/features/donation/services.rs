...
use crate::features::donation::errors::grouped_donation_history_count_error::GroupedDonationHistoryCountError;
...
pub trait DonationHistoryService: Send + Sync {
    ...
    ) -> Result<HashMap<String, u32>, GetDonationTransactionHistoryError>;
}
...
impl DonationHistoryService for CommonServiceImpl<dyn DonationHistoryAdminRepository> {
    ...
    async fn get_grouped_donation_transaction_history_count(
        ...
        donation_filter: DonationFilter,
        donation_grouping: DonationGrouping,
    ) -> Result<HashMap<String, u32>, GroupedDonationHistoryCountError> {
        match self
            .in_memory_repository
            .get_grouped_donation_transaction_history_count(&donation_filter, &donation_grouping)
            .await
        {
            Ok(grouped_transaction_history_count) => Ok(grouped_transaction_history_count),
            Err(_) => Ok(self
                .repository
                .get_grouped_donation_transaction_history_count(
                    &donation_filter,
                    &donation_grouping,
                )
                .await?),
