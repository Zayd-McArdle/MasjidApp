...
use masjid_app_api_library::shared::extractors::file_handler::file_deleter::DeleteError;
use masjid_app_api_library::shared::extractors::file_handler::file_uploader::UploadError;
...
pub async fn upsert_events(
    ...
    {
        Ok(url) => {
            request.metadata.event_details.image_url = Some(url);
        }
        Err(err) => {
            match err {
    ...
    if let Some(in_memory_repository) = state.repository_map.get(&DbType::InMemory) {
        upsert_event_result = in_memory_repository
            .upsert_event(Event::from(request.metadata.clone()))
            .await;
    }
    if upsert_event_result.is_err() {
        upsert_event_result = state
            ...
            .get(&DbType::MySql)
            .unwrap()
            .upsert_event(Event::from(request.metadata))
            .await;
    }
...
mod test {
    ...
    use axum::http::StatusCode;
    use masjid_app_api_library::features::events::{
        Event, EventDTO, EventDetails, EventStatus, EventType, EventsRepository, GetEventsError,
    };
    use masjid_app_api_library::shared::data_access::db_type::DbType;
    use masjid_app_api_library::shared::extractors::file_handler::FileHandler;
    use masjid_app_api_library::shared::extractors::request_validator::multipart::ValidatedMultipartRequest;
    ...
    async fn test_upsert_event() {
        ...
        let test_cases = [
            //Given the request metadata is invalid, I should get a bad request
            TestCase {
            ...
            // Given the metadata is valid, but event upsertion fails in database, I should get an internal server error
            TestCase {
            ...
            // Given the metadata is valid and upsertion succeeds, I should get an ok response
            TestCase {
                request: get_valid_upsert_request(false),
                file_uploader: FileHandler::default(),
